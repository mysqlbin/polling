...
def check_info(nowtime):
    try:
        #这里要连接从库
        con = mdb.connect(host='192.168.0.11',user='root',passwd='',db='consistency_db',port=3306)
        cur = con.cursor()
...
if __name__ == "__main__":
    ...
    #这里是连接主库
    cmd = "pt-table-checksum --nocheck-replication-filters --no-check-binlog-format --replicate=consistency_db.checksums h=,u=pt_user,p=123456abc,P=3306 --databases=niuniu_db"
    os.system(cmd)
